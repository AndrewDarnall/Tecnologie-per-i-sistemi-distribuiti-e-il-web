...
struct sockaddr* read_message(int socket_descriptor, char* receive_buffer, size_t chunk_size) {
    size_t total_bytes_read = 0;

    struct sockaddr* sender_address = malloc(sizeof(struct sockaddr_in));
    socklen_t address_length = sizeof(*sender_address);

    while(1) {
        ssize_t bytes_read = recvfrom(socket_descriptor, 
