...
@section('content')
<h1 class="display-1 sofia"> Edit Book Details </h1>
<br>
<br>
<form action="/books/{{$db->id}}" method="POST">
...
<br>
<form action="/books/{{$db->id}}" method="POST">
    <!-- Senza questo metodo non possiamo cancellare il record! -->
    {{csrf_field()}}
    @method('DELETE')
    <input class="form-control btn btn-primary" type="submit" value="Delete Book"/>
</form>
<br>
@endsection
@section('home')
